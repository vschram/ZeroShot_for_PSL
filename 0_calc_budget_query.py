...
def main():
    ytick_positions = np.linspace(0, 25, num=6, dtype=int)
    ...
    for scenario in ['al', 'random', 'largest', 'smallest']:
        ...
        plt.plot(accum_cost, runs, label=f'{name}')
    ...
    plt.savefig(f'Budget_spend_query.png')
    plt.savefig(f'Budget_spend_query.pdf')
    plt.show()
