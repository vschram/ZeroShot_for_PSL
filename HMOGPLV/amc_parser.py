...
## We inheriate this code from https://github.com/CalciferZh/AMCParser
## For us, we use the fucntion: read_line and parse_amc to parse the amc dataset.
## parse_amc can change the amc dataset into array and diction format. After that, we extract the useful data by ourselve.
...
class Joint:
    ...
    direction: Default direction of the joint(bone). The motions are all
        defined
    ...
    def set_motion(self, motion):
        if self.name == 'root':
            ...
            self.coordinate = self.parent.coordinate + self.length * \
                self.matrix.dot(self.direction)
