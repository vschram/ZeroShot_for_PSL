...
# from .kernels import Convolutional_test
...
### lmc kernel  ######
######################
# In lmc kernel, we have more than one kernel, is called base kernel (My own opinion).
...
def Kuf_lmc_separateiv(inducing_variable: SharedIndependentInducingVariables,
    ...
    Ws = [tf.reduce_sum(coreg.W, -1) for coreg in kernel.coregs]
    ## The we assign the w in here. If we want to change the a_d later, we will adjust it here for our new LMC model.
    ind = tf.cast(Xnew[:, -1], tf.int32)
    return tf.stack([tf.gather(W, ind) * Kuf for W, Kuf in zip(Ws, Kufs)])
...
def Kuu_kernel_test(inducing_variable: InducingPoints, kernel: Kernel, *, jitter=0.0):
    Kzz = kernel(inducing_variable.Z)
    # Kzz += jitter * tf.eye(len(inducing_variable), dtype=Kzz.dtype)
    Kzz += jitter * tf.eye(inducing_variable.num_inducing, dtype=Kzz.dtype)
