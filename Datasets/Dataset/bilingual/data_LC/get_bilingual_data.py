...
# Define folder path
for model in ["Transformer"]: #, "mBart"

    for metric in ["chrf", "bleu"]:
        if model == "mBart":
            path_model = r"mBart50\mBart50_ood_tseed2"
            LANG_dict = {'en': 17, 'id': 16, 'jv': 13, 'ms': 15, 'ta': 13, 'tl': 16}
        elif model == "Transformer":
            path_model = r"Transformer\Transformer_ood_tseed1"
            LANG_dict = {'en': 17, 'id': 15, 'jv': 12, 'ms': 14, 'ta': 12, 'tl': 15}
        folder_path = path_model

        end_filename = f"src_over_tgt_{model}_{metric}"
        ...
        count_tgt = -1
        idx_pref = -1
        for idx, src in enumerate(LANGS):
            print("NEW src", src,"idx:", idx, "idx_pref", idx_pref)
            ...
            for tgt in LANGS:
                ...
                if src == tgt:
                    array = np.zeros(LANG_dict[tgt], dtype=np.float32)
                    if idx == 0:
                        with open(f"{path}/{end_filename}_pdata.txt", 'w') as file:
                            values = np.round(np.log(ds[0:LANG_dict[tgt]]), 2)
                            for i in values:
                ...
                if count_tgt == len(LANGS)-1:

                    with open(f"{path}/{end_filename}.txt", 'a') as file:
                        values = array.tolist()
                        values = np.round(values[0:LANG_dict[tgt]], 2).astype(float)
